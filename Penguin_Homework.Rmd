...
#renv::snapshot()
#renv::restore ()
#please when attempting to run this code start with renv::restore in the console to ensure the required packages are loaded. 
```
...
write.csv (penguins_raw, here("penguins_raw.csv")) #This saves the data set to the same location as whereever this Rmd file is. I has it directed to a data folder within my R project, but I changed that as it would not allow the code to be run without makeing a specific data folder. 
...
write.csv (penguins_clean, here("penguins_clean.csv"))
...
png(filename="C:/Reproducible_figures/Figures/Flipper_and_body_mass.png") #This code saves the file as a png on my computer, the file route will have to be changed to run it on another computer, to your own figure file. 
plot(Fig.1)
dev.off()
...
png(filename="C:/Reproducible_figures/Figures/ANOVA_for_Flipper_and_body_mass.png") #Again this only works if you have this exact file route, to save the png to your own pc the file routing will have to be changed. 
plot(Table.2)
dev.off()
Table.2
```
